...
const AMENITIES = ["WiFi", "Food", "AC", "Laundry", "Parking", "Power Backup"];
...
// ✅ run API in background
if (isEdit) {
  savedPg = await apiFetch(`/owner/pgs/${pg._id}`, {
    method: "PATCH",
    body: data,
    isFormData: true,
  });
} else {
  savedPg = await apiFetch("/owner/pgs", {
    method: "POST",
    body: data,
    isFormData: true,
  });
...
// ✅ update UI instantly in dashboard
onSuccess(savedPg, isEdit);
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-black/40 backdrop-blur flex items-center justify-center px-4 overflow-y-auto"
      onClick={onClose}
    >
      <div
        className="w-full max-w-xl rounded-2xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-6"
        ...
        <div className="px-6 py-4 overflow-y-auto">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full rounded-xl bg-blue-600 py-3 text-white font-medium hover:bg-blue-700 transition disabled:opacity-60 disabled:cursor-not-allowed"
            >
